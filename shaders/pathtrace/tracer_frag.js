...
vec3 raytrace(in Ray ray, inout uint seed) {
    ...
    Material red_material = Material(vec3(0.1, 0.1, 0.7), vec3(0.0, 0.0, 0.0), vec3(0.0), 0.0, 0.0);
    Material green_material = Material(vec3(0.7, 0.7, 0.1), vec3(0.0, 0.0, 0.0), vec3(0.0), 0.0, 0.0);
    ...
    Sphere sphere3 = Sphere(vec3(0.0, 0.0, 3.35), 0.4, material3);
...
void get_color(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 jitter = vec2(rand_float(seed), rand_float(seed)) - 0.5;
    vec2 uv = (fragCoord + jitter) / vec2(512.0, 512.0);

    // create horizontal and vertical vectors
    float aspect_ratio = 512.0 / 512.0;
    vec3 horiz = vec3(1.0 * aspect_ratio, 0.0, 0.0);
    vec3 vert = vec3(0.0, 1.0, 0.0);
    
    // calculate ray direction
    vec3 look_from = vec3(0.0, 0.0, 0.1);
    ...
    uv = fragCoord / vec2(512.0, 512.0);
    vec3 last_color = texture(texture_1, uv).rgb;
    color = mix(last_color, color, texture_weight);
